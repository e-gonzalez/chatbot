...
def correct_string2 (ingredient_list, bot, update, user_data, markup2):
    ...
    if len(ingr_correction) > 0:
        if ingr_correction[0] == 'error':
            message = 'We are sorry. We could not find any correction to ' +str(ingr_list_client[0]) + '. \nCould you write it again, please?'
        ...
        return 'get_recipe', ingr_correct, ingr_correction, ingr_list_client
...
def correction(num_list, ingredients_list, bot, update, user_data):
  ...
  if correct_word == True :
        ...
        ingredient_correct = spoonacular_api_call('Get', url, parameters)
        ingredient_correct_json = str(ingredient_correct.json())
        if len(ingredient_correct.json()) == 0:
            ingr_correction.append('error')  
